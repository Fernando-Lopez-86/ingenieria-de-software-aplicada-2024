...
function PedidoEditForm() {
    ...
    const generatePDF = (data, items, formaPagoLabel, provinciaLabel) => {
        const doc = new jsPDF();
    ...
    const handleSubmit = (event) => {
        ...
        fetch('http://localhost:3000/api/pedidos/update', {
            ...
            .then((body) => {
                const formaPagoLabel = optionsFormasDePago.find(option => option.value === formData.CONDVENTA)?.label;
                const provinciaLabel = optionsProvincias.find(option => option.value === formData.PROENT)?.label;
                                    ...
                                    <tr key={item.ITEM} className="table-row">
                                        ...
                                        <td className="form-group column column-medium">
                                            ...
                                            <NumericFormat
                                                ...
                                                onValueChange={(values) => handleItemChangeFloat(index, values.value, 'PRECIO')}
                                                required 
                                                data-error="Por favor, ingrese el precio."
                                                maxLength={10}
                                            />
